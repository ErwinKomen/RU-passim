...
tokenizer = re.compile("({})".format( '|'.join(token[0] for token in tokens) ))
token_dict = {name: re.compile(token) for token, name in tokens}

# ============== code supporting the Parser ==============================
    ...
    def _parse_tree(self, text):
        ...
        try:
            ...
            if lp_count != rp_count:
                raise NewickError(
                    "Mismatch, {} open vs {} close parentheses.".format(lp_count, rp_count)
                )
            ...
            try:
                ...
                raise NewickError(
                    "Text after semicolon in Newick tree: {}".format(next_token.group())
                )
            ...
            response = Newick.Tree(root=root_clade, rooted=self.rooted)
        except:
            msg = oErr.get_error_message()
            oErr.DoError("_parse_tree")
        return response
...
def parse_newick(file, **kwargs):
    ...
    with File.as_handle(file) as fp:
        yield from Parser(fp).parse(**kwargs)
        # yield from newickio.parse(fp, **kwargs)
        # yield from Parser(fp, **kwargs)
    


def readnewick(file, **kwargs):
    """Parse a newick file and return a single tree.
    ...
    try:
        ...
        tree_gen = parse_newick(file, **kwargs)
        tree = next(tree_gen)
    except StopIteration:
...
def readnewick_string(str_input, **kwargs):
    """Parse a newick string and return a single tree.

    Raises a ``ValueError`` if there are zero or multiple trees -- if this
    occurs, use ``parse`` instead to get the complete sequence of trees.
    """
    try:
        tree_gen = Parser.from_string(str_input)
