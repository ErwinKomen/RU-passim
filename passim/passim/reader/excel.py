...
from passim.basic.views import app_editor
...
class ManuscriptUploadExcel(ReaderImport):
                ...
                for data_file in files:
                    ...
                    if data_file == None or data_file == "":
                        ...
                        if extension == "xlsx":
                            ...
                            if not self.upload_one_excel(tmp_path, filename, lst_err, oResult, kwargs, self.manucreate):
                                # We were not able to upload it correctly
                                oErr.Status("Could not upload Excel correctly")
                            else:
                                # Show that the upload worked
                                oErr.Status("The Excel has been correctly uploaded")
    ...
    def upload_one_excel(sPath, filename, lst_err, oResult, kwargs, manucreate=True):
        """Upload one Excel file at the indicated location"""

        oErr = ErrHandle()
        bResult = True
        try:
            # Read string file
            wb = openpyxl.load_workbook(sPath, read_only=True)
            sheetnames = wb.sheetnames
            ws_manu = None
            ws_sermo = None
            ws_codico = None
            ...
            if len(err_json) > 0:
                ...
                if ws_manu != None:
                    ...
                    # We have an object with key/value pairs: process it
                    kwargs['manucreate'] = manucreate
                    if manucreate and "manuscript_id" in oResult:
                        kwargs['manuscript_id'] = oResult['manuscript_id']
                    manu = Manuscript.custom_add(oManu, params, **kwargs)
